...
#import ruamel.yaml
import ipaddress
def networks(site_name,ilo_cidr,ilo_start,ilo_end,pxe_cidr,management_cidr,storage_cidr,calico_cidr,overlay_cidr,routable_cidr):

#def networks(site_name,ilo_cidr):
    yaml = YAML()
...
#    print(data)
    ...
    data[0]['data']['cidr'] = ilo_cidr
    data[0]['data']['routes'][0]['gateway'] = str(ilo_ips[1])
    data[0]['data']['ranges'][0]['start'] = ilo_start
    data[0]['data']['ranges'][0]['end'] = ilo_end
    ...
    data[2]['data']['cidr'] = management_cidr
    data[2]['data']['routes'][0]['gateway'] = str(management_ips[1])
    data[2]['data']['ranges'][0]['start'] = str(management_ips[1])
    data[2]['data']['ranges'][0]['end'] = str(management_ips[10])
    data[2]['data']['ranges'][1]['start'] = str(management_ips[11])
    data[2]['data']['ranges'][1]['end'] = str(management_ips[-2])
